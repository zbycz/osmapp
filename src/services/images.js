import { fetchText, removeFetchCache } from './helpers';

const getMapillaryUrl = async feature => {
  const lonlat = feature.center.map(x => x.toFixed(5)).join(',');
  const url = `https://a.mapillary.com/v3/images?client_id=TTdNZ2w5eTF6MEtCNUV3OWNhVER2dzpjMjdiZGE1MWJmYzljMmJi&lookat=${lonlat}&closeto=${lonlat}&per_page=2`;
