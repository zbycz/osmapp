...
import { getOsmUrl } from '../../../../services/osmApi';
...
const fetchFromOsmApi = async (osmId: OsmId) => {
  const { elements } = await fetchJson(getOsmUrl(osmId));
  return elements?.[0];
};
...
const getLastNodeLocation = async (osmId: OsmId, items: EditDataItem[]) => {
  ...
  const element = await fetchFromOsmApi(osmId);
